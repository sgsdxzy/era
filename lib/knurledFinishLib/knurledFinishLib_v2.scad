...
module knurled_cyl_str(chg, cod, cwd, cdp, fsh)
{
    cord=(cod+cdp)/2;
    cird=cord-cdp;
    cfn=round(2*cird*PI/cwd);
	 ...
	 if( fsh < 0 )
    {
        union()
        {
            shape(fsh, cird, cord, cfn*4, chg);


            knurled_finish_str(cord, cird, clf, chg, cfn);
        }
    }
    else if ( fsh == 0 )
    {
        intersection()
        {
            cylinder(h=chg, r=cord, $fn=2*cfn, center=false);
...
module knurled_finish_str(ord, ird, lf, sh, fn)
{
    for(i=[0:fn-1])
    {
        lf0=lf*i;
        ...
        polyhedron(
            points=[
                 [ 0,0,0],
                 [ ord*cos(lf0), ord*sin(lf0), 0],
                 [ ird*cos(lf1), ird*sin(lf1), 0],
                 [ ord*cos(lf2), ord*sin(lf2), 0],

                 [ 0,0,sh],
                 [ ord*cos(lf0), ord*sin(lf0), sh],
                 [ ird*cos(lf1), ird*sin(lf1), sh],
                 [ ord*cos(lf2), ord*sin(lf2), sh]
                ],
            faces=[
                 [0,1,2],[2,3,0],
                 [4,7,6],[6,5,4],   
